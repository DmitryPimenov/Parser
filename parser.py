...
class ParseVideoCard:
    ...
    async def parse(self):
        ...
        for page in range(1, 5):
            ...
            items = len(self.driver.find_elements_by_class_name("ProductCardCategoryList__list"))
            for item in range(items):
                cards = self.driver.find_elements_by_class_name("CatalogMenu__category-items js--CatalogMenu__category-items")
                for card in cards:
                    product_item = card.find_element_by_class_name("ProductCardHorizontal__properties_name")
